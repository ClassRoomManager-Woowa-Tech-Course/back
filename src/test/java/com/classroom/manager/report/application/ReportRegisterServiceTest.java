...
class ReportRegisterServiceTest {
    ...
    void delete_Success() {
        Long reportId = 42L;
        List<File> mockFiles = List.of(mock(File.class));
        when(fileService.findFilesByReportIdAndRelatedType(reportId, FileRelatedType.REPORT))
                .thenReturn(mockFiles);

        reportRegisterService.delete(reportId);

        InOrder inOrder = inOrder(fileService, reportRepository);
        inOrder.verify(fileService, times(1)).deleteAllFiles(mockFiles);
    ...
    void findReports_Success() {
        ...
        when(mockReport1.to()).thenReturn(response1);
        when(mockReport2.to()).thenReturn(response2);
        ...
        verify(mockReport1, times(1)).to();
        verify(mockReport2, times(1)).to();
    ...
    void findReport_Success() {
        ...
        when(mockReport.to()).thenReturn(mockResponse);
